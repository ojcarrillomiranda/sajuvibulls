...
<head>
    ...
    <title>Hstorias Clinicas fecha y hora pdf</title>
    <style>
          @page {
            margin: 0cm 0cm;
            font-family: Arial;
            ...
            <div class="row w-100">
                <div class="col show ">
                    <div class="table-responsive">
                        <div class="table-responsive">
                            <h3 id="paciente" class="text-black" style="margin-top: 80">Historias Clinicas fecha y hora de ingreso</h3>
                                    ...
                                    <tbody>
                                        @foreach ($informes as $informe)
                                            <tr>
                                                ...
                                                <td>{{ $informe->paciente->nombre }}</td>
                                                <td>{{ $informe->representante->primer_nombre }} {{ $informe->representante->primer_apellido }}</td>
                                                <td>{{ $informe->created_at }}</td>
                                                <td>{{ $informe->estado }}</td>
                                                <td>{{ $informe->tipoConsulta->descripcion }}</td>
                                                <td>{{ $informe->medico->name }} {{ $informe->medico->apellido }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            </table>
